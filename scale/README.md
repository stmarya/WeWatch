...
OS-level desktop actions are never executed on the signaling host. The
browser-only remote features (laser, annotation, in-app command, fullscreen,
TTS, and camera toggle) are routed to the selected client. OS-level mouse,
keyboard, process, clipboard, and workstation actions require a separately
authenticated native desktop agent with an explicit client target.
